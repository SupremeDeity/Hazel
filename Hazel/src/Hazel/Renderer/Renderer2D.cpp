...
namespace Hazel
{
	...
	void Renderer2D::Init()
	{
		...
		uint32_t offset = 0;
		for (uint32_t x = 0; x < s_Data.MaxIndexCount; x += 6)
		{
			QuadIndices[x + 0] = offset + 0;
			QuadIndices[x + 1] = offset + 1;
			QuadIndices[x + 2] = offset + 2;

			QuadIndices[x + 3] = offset + 2;
			QuadIndices[x + 4] = offset + 3;
			QuadIndices[x + 5] = offset + 0;

			offset += 4;
		}

		Ref<IndexBuffer> squareIB = IndexBuffer::Create(QuadIndices, s_Data.MaxIndexCount);
